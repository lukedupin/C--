/// \file getopt.cpp Provided by Professor Heckendorn
...
static int badopt( const char *name, const char *text)
{
...
int ourGetopt(int argc, char **argv, const char *opts)
{
    char *cp, ch;
    ...
    if (ch == ':' || (cp = strchr((char*)opts, ch)) == NULL) {
	if (argv[optind][++sp] == '\0') {
	    ++optind;
	    sp = 1;		/* to next word */
	}
